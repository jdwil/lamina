...
**Enum payloads.** An enum variant may be **unit** (a plain name), **tuple**-style (`Circle(i32)`), or **struct**-style (`Rect { w: i32, h: i32 }`). Payloads are **capability-gated**: a C-style target with no tagged unions `forbid`s the tuple/struct forms (a unit variant still renders everywhere), while a target with no payload-carrying `enum` (TypeScript) renders a payload-bearing enum as a discriminated union. A payload variant is **constructed** with existing expressions — a tuple variant via a call (`Circle(5)`), a struct variant via a struct literal — so no new construction node exists.
